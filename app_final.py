...
def process_movie_review(df, input_movie_text, review_text):
    ...
    if len(filtered_df) > 1:
        ...
        new_df["Cleaned_Review"] = remove_ner(new_df["Cleaned_Review"], nlp)
...
def main():
    ...
    if st.session_state.show_recommend_state:
        ...
        if result_type ==1:
            ...
            st.subheader(f"Top 10 Recommended Movies for :blue[{movie_info.iloc[0]['title']}]")
            st.dataframe(movie_list, hide_index=True)
        else:
            st.subheader(f"The movie :blue[{movie_rec}] doesn't exist in the records.")
    ...
    if st.session_state.show_match_state:
        ...
        st.subheader(f"Movie Reviews related to '{input_review}'")
        st.dataframe(result_df_2.drop(columns=['cosine_similarity'], axis=1), hide_index=True)
        ...
        if not result_df_2.empty:
            ...
            st.subheader(f"Top 10 :green[Positive] Reviews related to '{input_review}'")
            st.dataframe(result_df_tp_10_pos.drop(columns=['cosine_similarity', 'Sentiment'], axis=1), hide_index=True)

            # Filter and display top 10 negative reviews
            result_df_tp_10_neg = filter_top10_reviews(result_df_2, Sentiment.Negative.value)
            st.subheader(f"Top 10 :red[Negative] Reviews related to '{input_review}'")
            st.dataframe(result_df_tp_10_neg.drop(columns=['cosine_similarity', 'Sentiment'], axis=1), hide_index=True)

            # Filter and display top 10 negative reviews
            result_df_tp_10_neu = filter_top10_reviews(result_df_2, Sentiment.Neutral.value)
            st.subheader(f"Top 10 :grey[Neutral] Reviews related to '{input_review}'")
            st.dataframe(result_df_tp_10_neu.drop(columns=['cosine_similarity', 'Sentiment'], axis=1), hide_index=True)
    ...
    if st.session_state.show_movie_state:
        ...
        new_column_names = {'date': 'Review Date', 'review_sentiment': 'Sentiment'}
        result_df_movie.rename(columns=new_column_names, inplace=True)

        st.subheader(f"Movie Reviews on '{input_review_movie}'")
        st.dataframe(result_df_movie[['Review', 'Review Date', 'Sentiment']], hide_index=True)

        # Display a bar chart based on sentiment counts
        chart_data = pd.DataFrame(result_df_movie['Sentiment'].value_counts()).reset_index()
        chart_data.columns = ['Sentiment', 'Count']
